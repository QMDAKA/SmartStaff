...
public class PostITController {
    ...
    ResponseEntity<?> categories(
        ...
        if (query.length() < 2) {
            response = client.prepareSearch("viblo").setTypes("categories").setFrom(page * size).setSize(size) //
            ...
            response = client.prepareSearch("viblo").setTypes("categories").setQuery(qb)
    ...
    ResponseEntity<?> category(
        ...
        ObjectMapper mapper = new ObjectMapper();
        SearchResponse response = client.prepareSearch("viblo").setTypes(category)
    ...
    ResponseEntity<?> post(
        ...
        ObjectMapper mapper = new ObjectMapper();
        //QueryBuilder qb = rangeQuery("image","title").from(page);
        GetResponse response = client.prepareGet("viblo", category, postID).execute()
                .actionGet();
    ...
    ResponseEntity<?> search(
        ...
        SearchResponse response = client.prepareSearch("viblo").setQuery(queryBuilder).setFrom(page * size).setSize(size).addFields("image", "title", "name") //
                .addSort("createAt", SortOrder.DESC) //
                .execute().actionGet();
        SearchHits searchHits = response.getHits();
    ...
    ResponseEntity<?> searchByCatelogy(
        ...
        SearchResponse response = client.prepareSearch("viblo").setTypes(category).setQuery(qb)
                .setFrom(page * size).setSize(size).addFields("image", "title", "byCategory")
                .addSort("createAt", SortOrder.DESC)
                .execute().actionGet();
        SearchHits searchHits = response.getHits();
